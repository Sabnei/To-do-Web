...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (tagsInput) {
        const commonTags = ['trabajo', 'personal', 'urgente', 'proyecto', 'reunión', 'estudio', 'ejercicio', 'compras'];
        ...
        tagsInput.addEventListener('input', function() {
            ...
            if (lastTag.length > 0) {
                const suggestions = commonTags.filter(tag => 
                    tag.startsWith(lastTag) && !value.includes(tag)
                );
    ...
    function validateField(field) {
        ...
        if (field.name === 'descripcion') {
            if (value.length < 3) {
                ...
                showFieldError(field, 'La descripción debe tener al menos 3 caracteres');
            } else {
